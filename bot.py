...
async def Lucy_start():
    ...
    try:
        index_channels = await get_index_channels()
        logging.info('Runtime index channels loaded: %s', index_channels)
    except Exception as e:
        logging.exception('Could not load runtime index channels: %s', e)
    for name in files:
        with open(name) as a:
            patt = Path(a.name)
